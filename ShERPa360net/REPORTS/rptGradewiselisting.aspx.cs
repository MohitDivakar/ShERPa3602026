...
{
    public partial class rptGradewiselisting : System.Web.UI.Page
    {
        ...
        public static ChartData GetChartData(string frmdate, string todate, string plantid)
        {
            ...
            try
            {
                ...
                DataTable dt = new DataTable();
                dt = objMainClass.GetGradewidelisting(frmdate, todate, Convert.ToInt32(plantid));


                chartData.Labels = dt.AsEnumerable().Select(x => x.Field<string>("ReportDate")).ToArray();
                chartData.DatasetLabels = new string[] { "ACount", "BCount", "CCount" };
                List<int[]> datasetDatas = new List<int[]>();

                List<int> ACount = new List<int>();
                List<int> Bcounts = new List<int>();
                List<int> Ccounts = new List<int>();
                foreach (DataRow dr in dt.Rows)
                {
                    ACount.Add(Convert.ToInt32(dr["ACount"]));
                    Bcounts.Add(Convert.ToInt32(dr["BCount"]));
                    Ccounts.Add(Convert.ToInt32(dr["CCount"]));
                }

                datasetDatas.Add(ACount.ToArray());
                datasetDatas.Add(Bcounts.ToArray());
                datasetDatas.Add(Ccounts.ToArray());
