...
{
    public partial class frmFIVendMaster : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                try
                {
                    if (Session["USERID"] != null)
                    {
                        ...
                        string DOCNO = objMainClass.GETMAXVENDCODE(ddlVendType.SelectedValue, objMainClass.intCmpId, "SELECT", "0");
                        txtVendCode.Text = DOCNO;
            ...
            try
            {
                if (Session["USERID"] != null)
                {
                    if (ddlUPIWalletType.SelectedIndex > 0 || (txtIFSCCode.Text != "" && txtIFSCCode.Text != string.Empty))
                    {
                        ...
                        string DOCNO = objMainClass.InsertVendor(objMainClass.intCmpId, "", "", ddlVendType.SelectedValue, ddlTitle.SelectedValue, txtName1.Text, txtName2.Text, txtShortName.Text, 0, "", "", "",
                                   ...
                                   Convert.ToInt32(ddlState.SelectedValue), ddlCountry.SelectedValue, txtPincode.Text, txtName1.Text, txtContactNo.Text, txtContactNo.Text, REFTYPE,
                                   ADDOF, IDPROOF, PAN, CHEQUE, SHOP, 0, txtAadharNo.Text, Convert.ToInt32(ddlAccountType.SelectedValue),
                        ...
                        if (DOCNO != "" && DOCNO != string.Empty && DOCNO != null)
                        {
                            ...
                            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "myModal", "$('#modal-success').modal();$('#lblSuccessMsg').text(\"Vendor registered successfully! \");$('.close').click(function(){window.location.href ='frmFIViewVendorMaster.aspx' });", true);
                        }
                        else
                        {
