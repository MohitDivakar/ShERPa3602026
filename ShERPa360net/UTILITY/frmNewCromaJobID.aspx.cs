...
{
    public partial class frmNewCromaJobID : System.Web.UI.Page
    {
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                if (Session["USERID"] != null)
                {
                    ...
                    if (txtJobid.Text != "" && txtJobid.Text != null && txtJobid.Text != string.Empty)
                    {
        ...
        public int GetCount()
        {
            ...
            try
            {
                if (Session["USERID"] != null)
                {
                    for (int i = 0; i < dtNewList.Items.Count; i++)
                    {
                        DataListItem listItem = dtNewList.Items[i];
            ...
            try
            {
                if (Session["USERID"] != null)
                {
                    ...
                    if (response.StatusCode == HttpStatusCode.OK)
                    {
                        ...
                        ScriptManager.RegisterStartupScript(Page, Page.GetType(), "myModal", "$('#modal-success').modal();$('#lblSuccessMsg').text(\" " + msg + "\");$('.close').click(function(){window.location.href ='frmNewCromaJobID.aspx' });", true);
        ...
        protected void dtNewList_ItemDataBound(object sender, DataListItemEventArgs e)
        {
            try
            {
                if (Session["USERID"] != null)
                {
                    Label lblJOBID = (Label)(e.Item.FindControl("lblJOBID"));
                    Label lblAVAILSTAT = (Label)(e.Item.FindControl("lblAVAILSTAT"));
                    Label lblROWCOLOR = (Label)(e.Item.FindControl("lblROWCOLOR"));
                    CheckBox chkSelect = (CheckBox)(e.Item.FindControl("chkSelect"));

                    if (lblAVAILSTAT.Text == "RFD")
                    {
                        DataListItem gvMasterRow = (DataListItem)e.Item;
                        gvMasterRow.CssClass = lblROWCOLOR.Text;
        ...
        protected void lbkNLP_Click(object sender, EventArgs e)
        {
            try
            {
                if (Session["USERID"] != null)
                {
                    DataListItem grdrow = (DataListItem)((LinkButton)sender).NamingContainer;
                    Label lblDEALERPRICE = (Label)grdrow.FindControl("lblDEALERPRICE");
                    //lblDEALERPRICE.Style["display"] = "inline";

                    lblDEALERPRICE.CssClass = lblDEALERPRICE.CssClass.Replace("hidden", "").Trim();


                    string script = "setTimeout(function() {" +
                    "var labels = document.querySelectorAll('" + lblDEALERPRICE + "');" +
                    "labels.classList.add('hidden');" +
                    "}, 5000);";
                    ScriptManager.RegisterStartupScript(this, GetType(), "hideLabel", script, true);
        ...
        protected void chkSelect_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (Session["USERID"] != null)
                {
                    DataListItem grdrow = (DataListItem)((CheckBox)sender).NamingContainer;
                    ...
                    if (chkSelect.Checked == true)
                    {
                        ...
                        {
                            if (plantArray[i].Trim() == lblPLANTCD.Text)
                            {
                                PLantRights = lblPLANTCD.Text;
                            }
                        }
                    ...
                    for (int i = 0; i < dtNewList.Items.Count; i++)
                    {
                        DataListItem gvMasterRow = (DataListItem)dtNewList.Items[i];
                        Label lblAVAILSTAT = (Label)(gvMasterRow.FindControl("lblAVAILSTAT"));
                        Label lblROWCOLOR = (Label)(gvMasterRow.FindControl("lblROWCOLOR"));
                        CheckBox chkSelect1 = (CheckBox)gvMasterRow.FindControl("chkSelect");
                        if (lblAVAILSTAT.Text == "RFD")
                        {
                            gvMasterRow.CssClass = lblROWCOLOR.Text;
                            chkSelect1.Visible = true;
                        }
                        else
                        {
                            gvMasterRow.CssClass = lblROWCOLOR.Text;
                            chkSelect1.Visible = false;
                        }
                    }
                    //dtNewList.DataBind();
                    //Label lblJOBID = (Label)(grdrow.FindControl("lblJOBID"));
                    //Label lblAVAILSTAT = (Label)(grdrow.FindControl("lblAVAILSTAT"));
                    //Label lblROWCOLOR = (Label)(grdrow.FindControl("lblROWCOLOR"));

                    //if (lblAVAILSTAT.Text == "RFD")
                    //{
                    //    DataListItem gvMasterRow = (DataListItem)grdrow;
                    //    gvMasterRow.CssClass = lblROWCOLOR.Text;
                    //    chkSelect.Visible = true;
                    //}
                    //else
                    ...
                    //GridView grvData = (GridView)(grdrow.FindControl("grvData"));
                    //DataTable dt = new DataTable();
                    //dt = objMainClass.CheckRateJobDuplicate(objMainClass.intCmpId, lblJOBID.Text, "GETJOBIDSEARCH");
                    //grvData.DataSource = dt;
                    //grvData.DataBind();
